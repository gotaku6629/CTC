...
class Charades(data.Dataset):
    ...
    def prepare(self, rgb_path, labels, split, temporal, gap):
        ...
        # 全体の(予測)ラベル数を調査します
        #o_all_label = torch.IntTensor(self.o_classes).zero_()
        #v_all_label = torch.IntTensor(self.v_classes).zero_()
        #s_all_label = torch.IntTensor(self.s_classes).zero_()
        o_all_label = [0 for _ in range(self.o_classes)]
        v_all_label = [0 for _ in range(self.v_classes)]
        s_all_label = [0 for _ in range(self.s_classes)]

        rgb_image_paths, o_targets, v_targets, s_targets, o_f_targets, v_f_targets, s_f_targets, ids, o_times, v_times, s_times = [], [], [], [], [], [], [], [], [], [], []
        ...
        with open('./cr_caches/label_'+split+'_log.csv', 'w') as f:
            ...
            for i, (vid, label) in enumerate(labels.items()): # num_(kind of video)
                ...
                # 1. time label series (1. 予測用のラベルの始まり時刻,  2. CTC用の入力時刻内の全てのラベル時刻)
                ## 1.1 time label series (ラベルの始まりの時刻のみを見る) 
                start_time_series = []
                for x in label:
                    if x['start'] < n_time and x['start'] not in start_time_series: # check the contents                         
                        start_time_series.append(x['start'])
                start_time_series.sort()
                start_time_length = len(start_time_series)
                if start_time_length <= 1: continue  # ラベルが1つより少ない動画はSkip!! (予測できないからね!!)

                ## 1.2 time label series (CTC用の入力時刻内のラベル)
                time_series = []
                for x in label:
                ...
                #if time_length <= 1: continue  # ラベルが1つより少ない動画はSkip!

                ## 1.3 動画の時刻設定
                start_time = time_series[0]           # 動画における1つ目のラベルの始まり時刻
                start_n = math.ceil(start_time * FPS) # そのフレーム数 (小数点は切り上げ) 
                end_time = start_time_series[start_time_length-1] # 予測したい最後のラベル
                end_n = int(end_time * FPS)           # そのフレーム数 (小数点は切り下げ)
                ...
                if split == 'val_video':  # 2.1 val_video          
                    if end_n-1-temporal*(GAP+1)*STACK-1 <= 0: continue # マイナスになりうるので!!
                    spacing = np.linspace(start_n, end_n-1-temporal*(GAP+1)*STACK-1, testGAP) # 1つのビデオを10分割
                    gt_label = []
                    for loc in spacing:
                        now = loc/FPS
                        now_end = (loc+temporal*(GAP+1)*STACK)/FPS
                        ...
                        time_in_series = []
                        future_time = 0
                        for t in range(time_length):  # 入力時刻内のラベル
                            if now <= time_series[t] <= now_end:
                                time_in_series.append(time_series[t])
                        for t in range(start_time_length):  # 予測する別のラベル
                            if now_end < start_time_series[t]:
                                future_time = start_time_series[t]
                                time_in_series.append(start_time_series[t])
                                break
                        time_in_length = len(time_in_series)
                        if future_time == 0: continue
                        # ここ(↓)は削らずに固定させる!!!  入力時刻内のラベル遷移数の数による比較をしたいから... 
                        #if time_in_length -1 < self.num_trans: continue  # 入力ラベル遷移(予測を含まない)が少なかったら
                        if time_in_length > adjust_time:  continue        # CTCの計算的に... 

                        ## 2.2.3 予測(1時刻分)Accuracy用のラベル
                        o_f_target = torch.IntTensor(self.o_classes).zero_()
                        v_f_target = torch.IntTensor(self.v_classes).zero_()
                        for x in label: # label of 1 video
                            if x['start'] == future_time:   
                                o, v = cls2int(x['class'], self.c2ov)
                                o_f_target[o] = 1
                                v_f_target[v] = 1
                        s_f_target = label[0]['scene']                
                        o_f_targets.append(o_f_target)  # 予測Accuracy用ね
                        v_f_targets.append(v_f_target)
                        s_f_targets.append(s_f_target)

                        ## 2.1.3. CTC計算用の adjust_time分のラベル (予測ラベルも含む)
                        o_target = torch.IntTensor(adjust_time, self.o_classes).zero_()  # CTC計算用のラベル
                        ...
                        # 入力時刻内のラベル (予測はここでは含まなくて...)
                        for t in range(time_in_length-1):
                            for x in label: # label of 1 video
                        ...
                        # 予測時刻のラベル (ここではその時刻に始まるラベルのみを考えています!!)
                        for x in label: # label of 1 video
                            if x['start'] == future_time:
                                o, v = cls2int(x['class'], self.c2ov)
                                o_target[time_in_length-1, o] = 1
                                v_target[time_in_length-1, v] = 1
                        for o in range(self.o_classes):
                            o_target_10[time_in_length-1] += o_target[time_in_length-1, o]*2**o
                        for v in range(self.v_classes):
                            v_target_10[time_in_length-1] += v_target[time_in_length-1, v]*2**v
                        ...
                        for x in label:
                            if x['start'] == future_time:
                                o, v = cls2int(x['class'], self.c2ov)
                                s_target = label[0]['scene']
                                if [s_target, o, v] not in gt_label: 
                                    gt_label.append([s_target, o, v])
                        ...
                        ids.append(vid)
                        o_times.append(o_target_length)  # {Object, Verb, Scene}の3つで独立した長さのラベル(予測ラベルまで含む)
                    ...
                    writer.writerow(gt_label)
                    self.gt_table[vid] = gt_label



                else:  # 2.2 train, test(val)
                    for ii in range(start_n, end_n-1-temporal*(GAP+1)*STACK-1, 100):  # 間引きのオーダー:10
                        now = ii/FPS
                        now_end = (ii + temporal*(GAP+1)*STACK)/FPS
                        ...
                        time_in_series = []
                        future_time = 0
                        v_list = [] # 入力動画において, 新しい動作ラベルが入ってくるかどうかを確認する
                        v_only_onehot_target = torch.IntTensor(adjust_time).zero_()  # これでCTCを計算します！
                        t_count = 0
                        for t in range(time_length):  # 入力時刻内のラベル
                            if now <= time_series[t] <= now_end:  # 最終時刻のラベルも含めます
                                # 入力動画のVerbラベルにおいて, 次々と新しい動作が入ってくるものだけに限定!!
                                new_flag = 0
                                for x in label: # label of 1 video
                                    if x['start'] <= time_series[t] < x['end']:
                                        o, v = cls2int(x['class'], self.c2ov)
                                        if v not in v_list: # 新しいものが入ってくれば, 
                                            new_flag = 1
                                            v_list.append(v)
                                            v_only_onehot_target[t_count] = v  #  一番最初のマルチラベルは... 上書きされちゃうけど...
                                if new_flag == 1: # その時刻を追加する
                                    time_in_series.append(time_series[t])
                                    t_count += 1
                        ...
                        for t in range(start_time_length):  # 予測する別のラベル
                            ...
                            if now_end < start_time_series[t]:
                                ...
                                time_in_series.append(start_time_series[t])
                                break
                        time_in_length = len(time_in_series)  # 予測まで含めた遷移の長さ
                        if future_time == 0: continue  # 予測ラベルがなかったら Skip!!
                        if time_in_length -1 < self.num_trans:  continue  # 入力ラベル遷移(予測を含まない)が少なかったら
                        if time_in_length > adjust_time:  continue        # CTCの計算的に... 
                        ...
                        o_f_target = torch.IntTensor(self.o_classes).zero_()
                        #v_f_target = torch.IntTensor(self.v_classes).zero_()
                        #v_f_target = torch.IntTensor(1).zero_()
                        v_flag = 0 # break用 (今はがんばってVerbだけを見るよ!!)
                        for x in label: # label of 1 video
                            if x['start'] == future_time:   
                                o, v = cls2int(x['class'], self.c2ov)
                                # ラベル数のオーダーを揃えたいので, 多いものは追加しません！               
                                if v_all_label[v] > 50:
                                    v_flag = 1
                                    break
                                o_f_target[o] = 1
                                #v_f_target[0] = v
                                v_f_target = v
                                o_all_label[o] += 1  # 全体のObjectラベル数を確認
                                v_all_label[v] += 1  # 全体のVerbラベル数を確認
                        if v_flag == 1:
                            continue
                        s_f_target = label[0]['scene'] 
                        s_all_label[s_f_target] += 1  # 全体のSceneラベル数を確認

                        ## 2.2.4. CTC計算用の adjust_time分のラベル (予測ラベルも含む)
                        ...
                        if o_target_length < adjust_time:  # 現在の動画の時系列が, adjust_timeよりも小さかったら...
                            for pad in range(adjust_time - o_target_length):
                                o_only_target[o_target_length+pad] = -1
                        # ここだけ変更！
                        if t_count < adjust_time:  # 現在の動画の時系列が, adjust_timeよりも小さかったら...
                            for pad in range(adjust_time - t_count):
                                v_only_onehot_target[t_count+pad] = -1

                        if o_target_length == 0:  continue   # CTCにおけるゴミを削除
                        if v_target_length == 0:  continue

                        rgb_image_paths.append(rgb_temporal)
                        o_targets.append(o_only_target)  # CTC計算用のラベル {Object, Verb, Scene}3つの独立ラベル 
                        v_targets.append(v_only_onehot_target)
                        s_targets.append(s_target)
                        ids.append(vid)
                        o_times.append(o_target_length)  # {Object, Verb, Scene}の3つで独立した長さのラベル(予測ラベルまで含む)
                        v_times.append(t_count)
                        s_times.append(1)
                        o_f_targets.append(o_f_target)  # 予測Accuracy用ね
                        v_f_targets.append(v_f_target)
                        s_f_targets.append(s_f_target)
                        ...
                        for t in range(temporal):
                            ...
                            """
                            v_writer.append(self.v_name[v_only_onehot_target[t].to('cpu').detach().clone().numpy()])
                        writer.writerow(['Object', o_target_length])    
                        writer.writerow(o_writer)
                        writer.writerow(['Verb', t_count])
                        writer.writerow(v_writer)
                        writer.writerow(['Scene', self.s_name[scene]])
        
        with open('./cr_caches/count_'+split+'_labels.csv', 'w') as f:
            writer = csv.writer(f)
            writer.writerow(['Object'])
            writer.writerow(o_all_label)
            writer.writerow(['Verb'])
            writer.writerow(v_all_label)
            writer.writerow(['Scene'])
            writer.writerow(s_all_label)


        print('len(rgb_image_paths)={}'.format(len(rgb_image_paths)))
        print('len(o_targets)={}'.format(len(o_targets)))
        return {'rgb_image_paths': rgb_image_paths, 'o_targets': o_targets, 'v_targets': v_targets, 's_targets': s_targets, 'o_f_targets': o_f_targets, 'v_f_targets': v_f_targets, 's_f_targets': s_f_targets, 'ids': ids, 'o_times': o_times, 'v_times': v_times, 's_times': s_times}
    ...
    def __getitem__(self, index):
        ...
        o_target = self.data['o_targets'][index]  # CTC計算用 (adjust_time分の{O,V,S}が独立したラベル)
        v_target = self.data['v_targets'][index]
        s_target = self.data['s_targets'][index]
        o_f_target = self.data['o_f_targets'][index]  # 予測Accuracy測定用のラベル
        v_f_target = self.data['v_f_targets'][index]
        s_f_target = self.data['s_f_targets'][index]
        meta = {}
        meta['id'] = self.data['ids'][index]
        meta['o_time'] = self.data['o_times'][index]  # {Object, Verb, Scene} 3つの独立したラベルの長さ
        ...
        return rgb_temporal_image, o_target, v_target, s_target, o_f_target, v_f_target, s_f_target, meta
...
def get(args):
    ...
    print('charades_ctc_next_pred ready ...')
    ...
    return train_dataset, val_dataset, valvideo_dataset, valvideo_dataset.gt_table
