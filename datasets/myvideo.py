            ...
            for i, (vid, label) in enumerate(labels.items()): # num_(kind of video)
                ...
                for t in range(time_length): # [0.0, 11.9, 12.6, 21.2]
                    ...
                    for x in label: # label of 1 video
                        ...
                        if x['start'] == time_series[t]:  # 同じstart_timeのラベルは上書きされる
                            o, v = cls2int(x['class'], self.c2ov) # o, v = c2ov[int(x[1:])]
                            #print('c={}, o={}, v={}'.format(c_target[t], o+1, v+1))
                            o_target[t] = o+1
                            v_target[t] = v+1                            
                            time_end = x['end']
                ...
                if time_length < adjust_time:  # 現在の動画の時系列が, adjust_timeよりも小さかったら...
                    for pad in range(adjust_time - time_length): 
                        o_target[time_length+pad] = 0
                        v_target[time_length+pad] = 0
        
                ## s is single label
                s_target[0] = label[0]['scene']+1  # blankラベル分
                ...
                #writer.writerow(['c_label', c_target[0].to('cpu').detach().clone().numpy(), c_target[1].to('cpu').detach().clone().numpy(), c_target[2].to('cpu').detach().clone().numpy(), c_target[3].to('cpu').detach().clone().numpy(), c_target[4].to('cpu').detach().clone().numpy(), c_target[5].to('cpu').detach().clone().numpy(), c_target[6].to('cpu').detach().clone().numpy(), c_target[7].to('cpu').detach().clone().numpy()])
