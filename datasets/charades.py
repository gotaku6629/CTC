...
class Charades(data.Dataset):
    ...
    def prepare(self, rgb_path, labels, split, temporal, gap):
        ...
        for i, (vid, label) in enumerate(labels.items()): # num_(kind of video)
            ...
            o_target = torch.IntTensor(time_length-1, self.o_classes).zero_()
            v_target = torch.IntTensor(time_length-1, self.v_classes).zero_()
            s_target = torch.IntTensor(time_length-1).zero_()

            for t in range(len(time_series)-1): # [0.0, 11.9, 12.6, 21.2]        
                for x in label: # label of 1 video
                    if x['start'] <= time_series[t] and x['end'] >= time_series[t+1]: # FPS=24
            ...
            #for j in range(len(label)):
                #print(label[j]['scene'], label[j]['class'], label[j]['start'], label[j]['end'])
                #csv_writer.writerow([label[j]['scene'], label[j]['class'], label[j]['start'], label[j]['end']])
            ...
            #print('s_targets={}'.format(s_targets))
            #print('o_targets={}'.format(o_targets))
            #print('v_targets={}'.format(v_targets))
    ...
    def __getitem__(self, index):
        ...
        #print('s_target = {}'.format(self.data['s_targets'][index]))
        #print('s_target.shape = {}'.format(self.data['s_targets'][index].shape))
        ...
        return rgb_temporal_image, s_target, o_target, v_target, meta
