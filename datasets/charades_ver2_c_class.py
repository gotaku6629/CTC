...
class Charades(data.Dataset):
    ...
    def prepare(self, rgb_path, labels, split, temporal, gap):
        ...
        print('max_length = {}'.format(max_length)) # train:21, test:22
        ...
        with open('label_'+split+'_log.csv', 'w') as f:
            ...
            for i, (vid, label) in enumerate(labels.items()): # num_(kind of video)
                ...
                # (1つの動画の長さ > (STACK*(GAP+1)) x temporal = 100 でないものはスキップされる!!)
                if n < (STACK*(GAP+1))*temporal:
                ...
                time_length = len(time_series)
                #time_length = len(time_limit_label)

                if n == 0:
                    #print('sorry this video is not length') # None
                    continue
                if time_length < 3: # 3遷移以上行動ラベルが変化する動画像において...
                ...
                for t in range(time_length):
                    if flag_time_over == 1: # 2つ目の予測はしない! 1つだけ!!
                        break
                    if time_series[t] > (STACK*(GAP+1))*temporal/FPS: # 240
                        flag_time_over = 1
                    time_limit_label.append(time_series[t])
                if time_limit_label[len(time_limit_label)-1] < (STACK*(GAP+1))*temporal/FPS: # もし予測ラベルが入ってなかったら...
                    continue
                print(time_series)
                print(time_limit_label)

                ## 2. get image x 10[time]       # 240フレームを入力として, 
                rgb_temporal = []
                ...
                ## 3. get labels {s,v,o,ids,time} # 240フレーム内に2遷移と, 240フレームの外の3フレーム目を正解とする動画ラベル
                #  - o and v are multilabel, now, is not myltilabel, but single lable based on 2 rules!!
                #o_target = torch.IntTensor(adjust_time).zero_()    # ready for adding padding
                #v_target = torch.IntTensor(adjust_time).zero_()
                s_target = torch.IntTensor(1).zero_() 
                c_target = torch.IntTensor(adjust_time).zero_()                

                #print('time_series={}'.format(time_series))
                
                for t in range(len(time_limit_label)): # [0.0, 11.9, 12.6, 21.2]
                    if t == adjust_time:   # ラベルの長さを減らしにかかります!!
                        break
                    for x in label: # label of 1 video
                        # 最初の時刻のみを見るから, 時刻系列の最後のほうは無視している! (ラベル数は減らせる)
                        if x['start'] == time_limit_label[t]:  # 同じstart_timeのラベルは上書きされる
                        #if x['start'] == time_series[t]:
                            c = int(x['class'][1:])
                            c_target[t] = c
                ...
                #writer.writerow([time_limit_label[2], time_limit_label[3]]) # 動作の2遷移目が何秒かを知りたい!
                writer.writerow([c_target[0].to('cpu').detach().clone().numpy(), c_target[1].to('cpu').detach().clone().numpy(), c_target[2].to('cpu').detach().clone().numpy(), c_target[3].to('cpu').detach().clone().numpy()])
                #writer.writerow(c_target)
        ...
        if split == 'train':
            ...
            print('len(rgb_image_paths)={}'.format(len(rgb_image_paths[:-1])))
            print('len(s_targets)={}'.format(len(s_targets[:-1])))
            print('len(c_targets)={}'.format(len(c_targets[:-1])))
            return {'rgb_image_paths': rgb_image_paths[:-1], 's_targets': s_targets[:-1], 'c_targets': c_targets[:-1], 'ids': ids[:-1], 'times': times[:-1]}
    ...
    def __getitem__(self, index):
        ...
        #print('data={}'.format(self.data['rgb_image_paths'][index]))
        #print('data[0]={}'.format(self.data['rgb_image_paths'][index][0]))
        #print('data[1]={}'.format(self.data['rgb_image_paths'][index][1]))
        #print('data[2]={}'.format(self.data['rgb_image_paths'][index][2]))
        ...
        #print('rgb_temporal_image={}'.format(rgb_temporal_image))
        #print('s_target={}'.format(s_target))
        #print('c_target={}'.format(c_target))
...
def get(args):
    ...
    train_dataset = Charades(
        args.rgb_data, 'train', train_file, args.cache_dir, temporal, gap, 
    ...
    val_dataset = Charades(
        args.rgb_data, 'val', val_file, args.cache_dir, temporal, gap,
    ...
    valvideo_dataset = Charades(
        args.rgb_data, 'val_video', val_file, args.cache_dir, temporal, gap,
        rgb_transform=transforms.Compose([
            transforms.Resize(int(256./224*args.inputsize)),
            transforms.CenterCrop(args.inputsize),
            transforms.ToTensor(),
