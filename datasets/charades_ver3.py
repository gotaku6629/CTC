...
class Charades(data.Dataset):
    ...
    def prepare_groundtruth(self, labels, split, temporal, gap):
        ...
        with open(split+'_log.csv', 'w') as f:
            ...
            for i, (vid, label) in enumerate(labels.items()):
                ...
                # 2. adjust time series  # temporal*STACK*(GAP+1)までの時間にしたい!! 
                start_time = time_series[0]
                adjust_series = []
                for t in range(time_length):
                    if time_series[t] > start_time+temporal*STACK*(GAP+1)/FPS: break
                    adjust_series.append(time_series[t])
                adjust_length = len(adjust_series)
                time_finish = adjust_series[adjust_length-1]  # 本当は1種類のビデオに対して10個ぐらい取りたい!!
                ...
                for x in label:
                    if x['start'] <= time_finish and time_finish <= x['end']:
    ...
    def prepare(self, rgb_path, labels, split, temporal, gap):
        print('Charades prepare_ver3')
        """やること
            ・Paddingによる系列の長さ調整
                - 各ビデオのラベルの長さをゲットして, もっとも長い長さに全調整
                - 同時に各ビデオにおける長さを記録 → meta情報の'time'に記録する
        """
        FPS, GAP, testGAP = 24, gap, self.testGAP  # defalut
        ...
        with open('label_'+split+'_log.csv', 'w') as f:
            ...
            for i, (vid, label) in enumerate(labels.items()): # num_(kind of video)
                ...
                # 1.1 (1つの動画の長さ) > t*STACK*(GAP+1) = 200 でないものはスキップされる!!
                if n < start_time*FPS + temporal*STACK*(GAP+1) + 1: continue  #print('skip the video : {}, len = {}'.format(vid, n))

                # 1.2 入力時刻内のラベルとその1個先のラベルを推定したい
                # → end_timeまでにtime_seriesの要素が複数ある動画を採用する!!
                future_time = 0
                time_in_series = []
                for t in range(time_length):
                    ...
                    if time_series[t] > end_time:
                        ...
                        break
                time_in_length = len(time_in_series)  # 入力時刻内のラベル遷移 + 予測ラベル
                if future_time == 0:  continue
                if time_in_length -1 < self.num_trans: continue  # 入力時刻内のラベル遷移が少なかったら...
                if time_in_length > adjust_time: continue  # 動作遷移が10以上あるとCTCの計算ができなくなる...
                ...
                for t in range(temporal): # get 10 temporal
                    ...
                    #print('image file={}'.format(math.floor(start_time*FPS)+t*(GAP+1)*STACK+1))
                    rgb_temporal.append(rgb_impath)                    
                rgb_image_paths.append(rgb_temporal)


                ## 3. get future labels {s,v,o,ids,time}
                o_target = torch.IntTensor(self.o_classes).zero_() # Cross Entropyなので, 未来のラベルがあればよい!!
                ...
                for x in label: # label of 1 video
                    if x['start'] <= future_time and future_time <= x['end']:
                        o, v = cls2int(x['class'], self.c2ov)
                        o_target[o] = 1   # One-Hot形式で複数の正解をもたせる!!
                        v_target[v] = 1
                ## s is single label (int)
                s_target = label[0]['scene']
                #s_target[label[0]['scene']] = 1 # OneHot形式
                # csv出力デバック
                o_writer = []
                ...
                s_writer = []
                s_writer.append(self.s_name[s_target])
                writer.writerow(s_writer)
        ...
        if split == 'train':
            ...
            print('len(o_targets)={}'.format(len(o_targets[:-1])))
            return {'rgb_image_paths': rgb_image_paths[:-1], 'o_targets': o_targets[:-1], 'v_targets': v_targets[:-1], 's_targets': s_targets[:-1], 'ids': ids[:-1], 'times': times[:-1]}
