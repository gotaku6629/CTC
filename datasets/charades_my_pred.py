...
class Myvideo(data.Dataset):
    ...
    # gap            = '0'
        ...
        print('2.1.1 my label (not use csv file)') # テキトーなラベル
    ...
    def prepare(self, rgb_path, labels, split, temporal, gap):
        ...
        FPS, GAP, testGAP = 29.94, gap, self.testGAP  # defalut
        ...
        with open('label_'+split+'_log.csv', 'w') as f:
            ...
            for i, (vid, label) in enumerate(labels.items()): # num_(kind of video)
                ...
                end_n = n           # 自分の動画だから最後まで見たい
                end_time = n_time   # 最後までの時間
                print('start={}, end={}'.format(start_n, end_n))
                #if end_n - start_n < temporal*(GAP+1)*STACK: continue  # 入力時刻より短い動画はSkip!
                #if end_n-1-temporal*(GAP+1)*STACK-1 < 10: continue


                for ii in range(start_n, end_n-1-temporal*(GAP+1)*STACK-1, 1):  # なんとなく10のGAPを持たせてみる!
                    now = ii/FPS                                                 # 10フレームに1回推定することになるね!!
                    ...
                    if time_in_length < adjust_time:  # 現在の動画の時系列が, adjust_timeよりも小さかったら...
                    ...
                    s_writer = []
                    s_writer.append(self.s_name[scene])
    ...
    def __getitem__(self, index):
        ...
        return rgb_temporal_image, o_target, v_target, s_target, meta, rgb_path
...
def get(args):
    ...
    my_val_dataset = Myvideo(
        args.rgb_my_data, 'my', my_file, args.cache_dir, temporal, gap, num_trans,
    ...
    return my_val_dataset # , groundtruth.gt_table
