...
def get_ov_predictions(inference_o, inference_v): 
    top_o_ind = np.argsort(inference_o)[-10:] 
    top_v_ind = np.argsort(inference_v)[-10:] 

    score = inference_o[top_o_ind, None] + inference_v[None, top_v_ind]   
    top_flat_ind = np.argsort(score, axis = None)[-100:]
    top_score = score.ravel()[top_flat_ind]
    
    top_o, top_v = np.unravel_index(top_flat_ind, score.shape)
    
    predictions = [(top_score[j], 
        (top_o_ind[top_o[j]], top_v_ind[top_v[j]])) 
        for j in range(top_score.size)]
