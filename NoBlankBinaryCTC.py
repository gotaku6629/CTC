...
class NoBlankBinaryCTC(nn.Module):
    def __init__(self):
        super(NoBlankBinaryCTC, self).__init__()
        self.zero_padding = -10000000000000.0 # 4.91044616699219
        self.bce_loss = nn.BCELoss()
        self.sigmoid = nn.Sigmoid()
        print('NoBlankBinaryCTC ready...')
    ...
    def computes_transition(self, t, prev_prob, path, path_length, cum_prob, y):
        ...
        #bce = torch.zeros(self.n_batch, self.max_path_length).cuda()
        for b in range(self.n_batch):
            for l in range(self.max_path_length):
                #prob[b][l] = prob[b][l] + y[b][path[b][l]]
                prob[b][l] = prob[b][l] - self.bce_loss(y[b], path[b][l])
                #for n in range(self.n_unit):
                #    bce[b][l] += path[b][l][n]*torch.log(y[b][n])
                #    bce[b][l] += (1-path[b][l][n])*torch.log(1-y[b][n])
                #bce[b][l] = bce[b][l] / self.n_unit
                #prob[b][l] += bce[b][l]
    ...
    def calc_trans(self, yseq, input_length, path, path_length):
        ...
        for t in range(self.max_input_length):
            for b in range(self.n_batch):
                for l in range(self.max_path_length):
                    prob[t][b][l] = self.bce_loss(yseq[t][b], path[b][l]) * (-1)# OK! まちがってないけど, pytorchのモジュールだとBatchサイズを含めるよ!
                    #for n in range(self.n_unit):
                    #    prob[t][b][l] +=  path[b][l][n]*torch.log(yseq[t][b][n])
                    #    prob[t][b][l] += (1-path[b][l][n])*torch.log(1-yseq[t][b][n])
                    #prob[t][b][l] = prob[t][b][l] / self.n_unit  # これで平均にします!!
