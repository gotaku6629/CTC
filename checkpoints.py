...
def load(args, base_model, logits_model, base_optimizer, logits_optimizer):
    if args.resume: # ./cr_caches/test/model.pth.tar
...
#def save(epoch, args, base_model, logits_model, base_optimizer, logits_optimizer):
    state = {
        'epoch': epoch + 1,
    ...
    if is_best:
        ...
        score_file(scores, "{}/model_best.txt".format(args.cache))
